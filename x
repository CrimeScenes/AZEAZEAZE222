...
local function isWithinViewEra(position)
    ...
    local fovHeight = getgenv().Forbidden.ViewEra.Height * 100
    local fovWidth = getgenv().Forbidden.ViewEra.Width * 100
...
local function UpdateFOV()
    ...
    local success, errorMsg = pcall(function()
        if Circle then
            Circle.Visible = getgenv().Forbidden.PointLock.Radius_Visibility
            Circle.Radius = getgenv().Forbidden.PointLock.Radius
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if getgenv().Forbidden.PointLock.ClosestPart then
            ...
            BodyPart = TargetPlayer.Character:FindFirstChild(getgenv().Forbidden.PointLock.HitPart)
        end
        ...
        if BodyPart then
            ...
            if getgenv().Forbidden.PointLock.Aligner then
                ...
                if humanoid then
                    ...
                    predictedPosition = BodyPart.Position + (moveDirection * getgenv().Forbidden.PointLock.Prediction)
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * getgenv().Forbidden.PointLock.Prediction)
            end
            
            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if getgenv().Forbidden.PointLock.SmoothnessEnabled then
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, getgenv().Forbidden.PointLock.Smoothness)
                else
